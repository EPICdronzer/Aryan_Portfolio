...
export default function AboutPage() {
  ...
  // Auto-play state
  const expAutoPlay = useRef(null);
  const philAutoPlay = useRef(null);
  const toolAutoPlay = useRef(null);
  const expInteracting = useRef(false);
  const philInteracting = useRef(false);
  const toolInteracting = useRef(false);
  const expTimeout = useRef(null);
  const philTimeout = useRef(null);
  const toolTimeout = useRef(null);

  // Memoized scroll-to-index helper — prevents stale closures in intervals
  const makeScrollTo = useCallback((refEl) => (idx) => {
  ...
  // Stable ref so intervals always call the latest version
  const makeScrollToRef = useRef(makeScrollTo);
  useEffect(() => {
    makeScrollToRef.current = makeScrollTo;
  }, [makeScrollTo]);

  // ── Experience auto-play ─────────────────────────────────────
  useEffect(() => {
    const EXP_COUNT = 5;
    expAutoPlay.current = setInterval(() => {
      if (!expInteracting.current) {
        setActiveIndexExperience((prev) => {
          const next = (prev + 1) % EXP_COUNT;
          makeScrollToRef.current(experienceScrollRef)(next);
          return next;
        });
      }
    }, 4500);
    return () => clearInterval(expAutoPlay.current);
  }, []);

  // ── Philosophy auto-play ─────────────────────────────────────
  useEffect(() => {
    const PHIL_COUNT = 4;
    philAutoPlay.current = setInterval(() => {
      if (!philInteracting.current) {
        setActiveIndexPhilosophy((prev) => {
          const next = (prev + 1) % PHIL_COUNT;
          makeScrollToRef.current(philosophyScrollRef)(next);
          return next;
        });
      }
    }, 5000);
    return () => clearInterval(philAutoPlay.current);
  }, []);

  // ── Toolkit auto-play ────────────────────────────────────────
  useEffect(() => {
    const TOOL_COUNT = 4;
    toolAutoPlay.current = setInterval(() => {
      if (!toolInteracting.current) {
        setActiveIndexToolkit((prev) => {
          const next = (prev + 1) % TOOL_COUNT;
          makeScrollToRef.current(toolkitScrollRef)(next);
          return next;
        });
      }
    }, 4500);
    return () => clearInterval(toolAutoPlay.current);
  }, []);

  // Pause helpers
  const pauseExp = () => {
    expInteracting.current = true;
    clearTimeout(expTimeout.current);
    expTimeout.current = setTimeout(() => {
      expInteracting.current = false;
    }, 4000);
  };
  const pausePhil = () => {
    philInteracting.current = true;
    clearTimeout(philTimeout.current);
    philTimeout.current = setTimeout(() => {
      philInteracting.current = false;
    }, 4000);
  };
  const pauseTool = () => {
    toolInteracting.current = true;
    clearTimeout(toolTimeout.current);
    toolTimeout.current = setTimeout(() => {
      toolInteracting.current = false;
    }, 4000);
  };
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ...
        { y: 50, opacity: 0 },
        {
          ...
          duration: 0.85,
          ease: 'power3.out',
          scrollTrigger: { trigger: '.about-banner', start: 'top 85%', once: true },
        }
      );
      ...
      gsap.fromTo(
        ...
        { y: 40, opacity: 0 },
        {
          ...
          stagger: 0.12,
          ease: 'power3.out',
          scrollTrigger: { trigger: '.about-exp-card', start: 'top 85%', once: true },
        }
      );
      ...
      gsap.fromTo(
        ...
        { y: 30, opacity: 0 },
        {
          ...
          duration: 0.7,
          ease: 'power3.out',
          scrollTrigger: { trigger: '.about-philosophy-heading', start: 'top 85%', once: true },
        }
      );
      ...
      gsap.fromTo(
        ...
        { x: -60, opacity: 0 },
        {
          ...
          stagger: 0.15,
          ease: 'power3.out',
          scrollTrigger: { trigger: '.about-philosophy-panel', start: 'top 85%', once: true },
        }
      );
      ...
      gsap.fromTo(
        ...
        { scale: 0.88, opacity: 0, y: 30 },
        {
          ...
          stagger: 0.1,
          ease: 'back.out(1.4)',
          scrollTrigger: { trigger: '.about-toolkit-card', start: 'top 85%', once: true },
        }
      );
      ...
      gsap.fromTo(
        ...
        { y: 40, opacity: 0 },
        {
          ...
          stagger: 0.15,
          ease: 'power3.out',
          scrollTrigger: { trigger: '.about-timeline-item', start: 'top 85%', once: true },
        }
      );
    }, pageRef);
  ...
  // Scroll handlers — stopPropagation prevents page scroll contamination
  ...
  const timeline = [
    {
      ...
      desc: 'Began freelance editing, focusing on traditional editing pipelines in Adobe Premiere and understanding narrative pacing.',
    },
    {
      year: '2022',
      title: 'The Pacing Breakthrough',
      desc: 'Shifted focus to short-form retention metrics, developing frameworks to maximize watch time. Scaled multiple client videos past 5M+ views.',
    },
    {
      year: '2024',
      title: 'Generative AI Integration',
      desc: 'Pioneered integration of DaVinci Resolve workflows with advanced AI generative video models (Kling, Sora, Midjourney) for B-roll composition.',
    },
    {
      year: '2026',
      title: 'Elite Strategy & Consultation',
      desc: 'Partnering directly with leading Indian creators and tech founders to architect end-to-end viral video channels.',
    },
  ];
  ...
  return (
    <div ref={pageRef} className="relative pt-32 pb-16 px-6 md:px-20 lg:px-32 text-white overflow-hidden">
      <FloatingObjects id="about-page" variant="both" />
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-12 md:space-y-16">
        ...
        {/* Header Block */}
        <div className="about-header text-center max-w-3xl mx-auto space-y-4">
          ...
          <p className="text-zinc-400 text-sm md:text-base font-light leading-relaxed max-w-2xl mx-auto pt-2">
            Fusing cutting-edge generative AI technologies with narrative-driven editing psychology to craft
            viral digital experiences for brands and founders.
          </p>
        </div>

        {/* About Banner */}
        <div className="about-banner relative rounded-3xl overflow-hidden border border-zinc-800/60 shadow-[0_24px_80px_rgba(0,0,0,0.6)] bg-zinc-950/40 select-none pointer-events-none h-72 md:h-64">
          <img
          ...
          {/* Mobile layout */}
          <div className="md:hidden absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 py-8">
            <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-[#22d3ee]/50 shadow-[0_0_24px_rgba(34,211,238,0.25)] shrink-0">
              <img src="/img2.png" alt="Aryan Chopra" className="w-full h-full object-cover filter grayscale contrast-110" />
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#0c0c0e]/80 border border-emerald-500/30">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest">Available for Projects</span>
            </div>
            <div className="text-center space-y-1">
              <h3 className="text-2xl font-black text-white uppercase tracking-wide leading-tight">Aryan Chopra</h3>
              <p className="text-[#22d3ee] text-[10px] font-bold uppercase tracking-wider">AI Video Editor · Post-Production Specialist</p>
            </div>
          </div>

          {/* Desktop layout */}
          <div className="hidden md:flex absolute inset-0 items-end p-8 gap-6">
            <div className="w-28 h-28 rounded-2xl overflow-hidden border-2 border-[#22d3ee]/40 shadow-[0_0_20px_rgba(34,211,238,0.15)] shrink-0">
              <img src="/img2.png" alt="Aryan Chopra" className="w-full h-full object-cover filter grayscale contrast-110" />
            </div>
            <div className="space-y-1">
              ...
              <h3 className="text-2xl font-black text-white uppercase tracking-wide">Aryan Chopra</h3>
        ...
        <div className="relative space-y-8">
          ...
          {/* Floating decorative objects — desktop only */}
          <div className="hidden lg:block absolute top-[6%] left-[1%] animate-float-all-1 z-0 pointer-events-none">
            <svg width="90" height="90" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="opacity-35 filter grayscale drop-shadow-[0_8px_16px_rgba(255,255,255,0.05)]">
              <defs>
              ...
              <path d="M 100 30 C 138.66 30, 170 61.34, 170 100 C 170 138.66, 138.66 170, 100 170 C 61.34 170, 30 138.66, 30 100 C 30 61.34, 61.34 30, 100 30 Z M 100 65 C 119.33 65, 135 80.67, 135 100 C 135 119.33, 119.33 135, 100 135 C 80.67 135, 65 119.33, 65 100 C 65 80.67, 80.67 65, 100 65 Z" fill="url(#expTorusGrad)" />
            </svg>
          </div>

          <div className="hidden lg:block absolute top-[45%] right-[2%] animate-float-all-2 z-0 pointer-events-none">
            <svg width="70" height="70" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="opacity-30 filter grayscale drop-shadow-[0_8px_16px_rgba(255,255,255,0.05)]">
              <defs>
          ...
          <div className="hidden lg:block absolute bottom-[4%] left-[3%] animate-float-all-2 z-0 pointer-events-none">
            <svg width="80" height="80" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="opacity-35 filter grayscale drop-shadow-[0_8px_16px_rgba(255,255,255,0.05)]">
              <defs>
          ...
          <div className="hidden xl:block absolute top-[75%] right-[1%] animate-float-all-1 z-0 pointer-events-none">
            <svg width="60" height="60" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="opacity-30 filter grayscale drop-shadow-[0_8px_16px_rgba(255,255,255,0.05)]">
              <defs>
              ...
              <path d="M50,100 C30,70 60,30 100,30 C140,30 170,70 150,100 C130,130 160,170 100,170 C40,170 70,130 50,100 Z" fill="none" stroke="url(#expSpiralGrad)" strokeWidth="28" strokeLinecap="round" />
            </svg>
          </div>

          <div className="text-center max-w-xl mx-auto space-y-2 relative z-10">
            <span className="text-[10px] font-black tracking-[0.3em] text-[#22d3ee] uppercase block">Career Path</span>
          ...
          <div className="hidden md:block max-w-3xl mx-auto space-y-4 relative z-10">
            {experience.map((job, idx) => (
              <div
                key={idx}
                className={`about-exp-card relative p-6 md:p-7 rounded-2xl bg-[#0c0c0e]/80 border transition-all duration-300 group ${
                  job.current
              ...
              >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <p className="text-xs md:text-[13px] text-zinc-400 font-light leading-relaxed pt-1">{job.desc}</p>
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {job.skills.map((skill, sIdx) => (
                        <span key={sIdx} className="text-[9px] font-medium bg-zinc-950/80 border border-zinc-800/80 px-2 py-0.5 rounded text-zinc-500 font-mono tracking-tight group-hover:border-[#22d3ee]/20 group-hover:text-zinc-400 transition-colors">
                          {skill}
                        </span>
                      ))}
                    </div>
          ...
          <div className="md:hidden relative z-10">
            <div
              ...
              onTouchStart={pauseExp}
              onMouseDown={pauseExp}
                ...
                <div
                  ...
                  className={`snap-center shrink-0 w-[82%] h-[280px] p-5 rounded-2xl bg-[#0c0c0e]/90 border flex flex-col transition-all duration-500 ease-out relative overflow-hidden
                    ${idx === activeIndexExperience
                      ? 'scale-100 opacity-100 border-[#22d3ee]/50 shadow-[0_0_30px_rgba(34,211,238,0.15)] z-20'
                      : 'scale-[0.92] opacity-40 border-zinc-800/60 z-10'
                    }
                  `}
                >
                  {job.current && (
                  ...
                  <h4 className="text-sm font-bold text-zinc-100 leading-snug h-9 overflow-hidden">{job.title}</h4>
                  ...
                  <p className="text-[11px] text-zinc-400 font-light leading-relaxed mt-2.5 flex-1 overflow-hidden">{job.shortDesc}</p>
                  <div className="flex flex-wrap gap-1.5 pt-3 mt-auto">
                    {job.skills.slice(0, 2).map((skill, sIdx) => (
                      <span key={sIdx} className="text-[9px] font-medium bg-zinc-950/80 border border-zinc-800/80 px-2 py-0.5 rounded text-zinc-500 font-mono tracking-tight">
                        {skill}
                      </span>
                    ))}
                  </div>
            ...
            <div className="flex justify-center gap-1.5 pt-4">
              {experience.map((_, idx) => (
                <span
                  key={idx}
                  className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeIndexExperience ? 'w-5 bg-[#22d3ee]' : 'w-1.5 bg-zinc-700'}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="space-y-12">
          <div className="max-w-2xl mx-auto text-center space-y-5 border-t border-b border-zinc-800/60 py-8">
            ...
            <p className="text-zinc-200 font-light leading-relaxed text-sm md:text-base">
              I believe video editing is not just about cuts and transitions—it is a branch of behavioral psychology.
              In a digital world dominated by short attention spans, the first three seconds are sacred ground.
              We design our clips using structural hook frameworks that keep viewers locked from the start.
            </p>
            <p className="text-zinc-400 font-light leading-relaxed text-xs md:text-sm">
              By combining state-of-the-art post-production suites (DaVinci Resolve, AE) with generative AI platforms,
              I act as a force multiplier. This workflow allows for rapid asset creation, automated layout composition,
              and high-fidelity soundscapes that would take typical production houses weeks to execute.
            </p>
          </div>

          {/* Desktop: alternating panel stack */}
                ...
                return (
                  <div key={idx} className="relative grid grid-cols-2 gap-3 items-stretch group">
                    ...
                    <div className={`relative h-44 rounded-2xl overflow-hidden border border-zinc-800/80 group-hover:border-[#22d3ee]/30 transition-all duration-500 ${reversed ? 'order-2' : 'order-1'}`}>
                      <img src={item.bg} alt={item.title} className="absolute inset-0 w-full h-full object-cover opacity-70 group-hover:opacity-90 group-hover:scale-105 transition-all duration-700 filter grayscale contrast-110" />
                    ...
                    <div className={`relative flex flex-col justify-center p-6 md:p-8 rounded-2xl border border-zinc-800/80 bg-[#0c0c0e]/80 group-hover:border-[#22d3ee]/30 group-hover:bg-[#121216]/60 transition-all duration-500 space-y-2 ${reversed ? 'order-1' : 'order-2'}`}>
                      <span className="text-[10px] font-mono text-[#22d3ee]/70 font-bold tracking-widest uppercase">{item.tag}</span>
                      <h4 className="text-lg md:text-xl font-black text-zinc-100 uppercase tracking-wide group-hover:text-[#22d3ee] transition-colors duration-300">{item.title}</h4>
                      <p className="text-xs md:text-[13px] text-zinc-400 font-light leading-relaxed">{item.desc}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {/* Mobile: swipeable philosophy cards */}
          <div
            ref={philosophyScrollRef}
            onScroll={handleScrollPhilosophy}
            onTouchStart={pausePhil}
            onMouseDown={pausePhil}
            style={{ overscrollBehaviorX: 'contain' }}
              ...
              <div
                ...
                className={`snap-center shrink-0 w-[85%] p-6 rounded-2xl bg-[#0c0c0e]/90 border shadow-[0_10px_30px_rgba(0,0,0,0.6)] relative overflow-hidden group min-h-[160px] flex flex-col justify-between cursor-default transition-all duration-500 ease-out
                  ${idx === activeIndexPhilosophy
                    ? 'scale-100 opacity-100 border-[#22d3ee]/50 shadow-[0_0_30px_rgba(34,211,238,0.18)] z-20'
                    : 'scale-[0.91] opacity-40 border-zinc-800/60 pointer-events-none z-10'
                  }
                `}
              >
                <img src={item.bg} alt={item.title} className="absolute inset-0 w-full h-full object-cover opacity-25 group-hover:opacity-45 group-hover:scale-105 transition-all duration-500 z-0 pointer-events-none select-none filter contrast-125" />
                ...
                <div className="relative z-10 flex flex-col justify-between h-full space-y-4">
                  <span className="text-[10px] font-mono text-[#22d3ee]/80 font-bold tracking-widest uppercase">{item.tag}</span>
                  <div className="space-y-1.5">
                    <h4 className="text-base font-black text-zinc-100 uppercase tracking-wide group-hover:text-[#22d3ee] transition-colors duration-300">{item.title}</h4>
          ...
          {/* Philosophy dot indicators */}
          <div className="md:hidden flex justify-center gap-1.5 pt-2">
            {philosophy.map((_, idx) => (
              <span
                key={idx}
                className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeIndexPhilosophy ? 'w-5 bg-[#22d3ee]' : 'w-1.5 bg-zinc-700'}`}
              />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-10">
          ...
          <div
            ...
            onTouchStart={pauseTool}
            onMouseDown={pauseTool}
            style={{ overscrollBehaviorX: 'contain' }}
          ...
          >
            {tools[activeCategory].map((tool, idx) => (
              <div
                key={idx}
                className={`about-toolkit-card snap-center shrink-0 w-[85%] aspect-square sm:w-[48%] md:w-auto md:shrink md:aspect-square p-6 rounded-2xl bg-[#0c0c0e]/90 border hover:bg-[#121216]/90 hover:shadow-[0_12px_35px_-6px_rgba(34,211,238,0.15)] hover:translate-y-[-4px] transition-all duration-500 ease-out relative overflow-hidden group flex flex-col justify-between
                ...
                `}
              >
                <img src={tool.bg} alt={tool.name} className="absolute inset-0 w-full h-full object-cover opacity-45 group-hover:opacity-65 group-hover:scale-105 transition-all duration-500 z-0 pointer-events-none select-none filter contrast-125" />
          ...
          {/* Toolkit dot indicators (mobile only) */}
          <div className="md:hidden flex justify-center gap-1.5 pt-2">
            {tools[activeCategory].map((_, idx) => (
              <span
                key={idx}
                className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeIndexToolkit ? 'w-5 bg-[#22d3ee]' : 'w-1.5 bg-zinc-700'}`}
              />
            ))}
          </div>
        </div>
          ...
          <div className="hidden md:block max-w-3xl mx-auto relative">
            ...
            <div className="space-y-12">
              {timeline.map((item, idx) => (
                <div key={idx} className={`about-timeline-item relative flex flex-row items-start ${idx % 2 === 0 ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className="w-1/2 px-8 relative z-10">
                    <div className="relative p-6 rounded-2xl bg-[#0c0c0e]/80 border border-zinc-800/80 hover:border-[#22d3ee]/30 transition-all duration-500 space-y-2 overflow-hidden group hover:shadow-[0_8px_30px_rgba(34,211,238,0.08)]">
                      <img src={timelineImages[idx]} alt={item.title} className="absolute inset-0 w-full h-full object-cover opacity-15 group-hover:opacity-30 group-hover:scale-105 transition-all duration-700 z-0 pointer-events-none select-none filter grayscale contrast-125" />
